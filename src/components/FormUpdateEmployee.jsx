...
const FormUpdateEmployee = () => {
  ...
  useEffect(() => {
    axios.get(`${API_BASE_URL}/api/employee/${id}`) // GET ONE EMPLOYEE
      .then(datos => {
        setFormData(datos.data)
      })
      .catch(error => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.put(`${API_BASE_URL}/api/employee/${id}`, formData)
      .then(response => {
        // console.log('Server response: ', response.data)
        // setModalTitle('Action Completed')
      ...
      .catch(error => {
        ...
        setModalText('Error updating employee. Please try again.')
        setShowModal(true)
      })
  }
